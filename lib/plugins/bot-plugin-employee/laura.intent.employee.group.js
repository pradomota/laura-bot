    ...
    http.get(`http://gsb.devel.e-paths.com/role/${encodeURIComponent(entityName)}`, (res) => {
        ...
        res.on('end', () => {
            try {
                ...
                let choicesKeyboard = new bot_core_1.BotBuilderExt.Keyboard(session).buttons(employees.map(employee => bot_core_1.BotBuilder.CardAction.imBack(session, employee.fullname, session.gettext('suggestion.employee.head.value', employee.fullname))
