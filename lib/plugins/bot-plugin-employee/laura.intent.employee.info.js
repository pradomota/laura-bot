    ...
    http.get(`http://gsb.devel.e-paths.com/person/${encodeURIComponent(entityName)}`, (res) => {
        ...
        res.on('end', () => {
            try {
                ...
                let suggestions = new bot_core_1.BotBuilderExt.Keyboard(session).buttons([card(session, 'suggestion.employee.boss')]).toAttachment();
...
function card(session, msgidPrefix) {
    return bot_core_1.BotBuilder.CardAction.imBack(session, session.gettext(msgidPrefix + '.title'), msgidPrefix + '.value');
}
